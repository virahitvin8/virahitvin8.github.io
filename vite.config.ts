...
/*
 * The Figma Make plugins below (site configuration, error-overlay replay,
 * refresh-boundary fallback, kit stories) only exist inside Figma Make, where
 * the platform generates `.figma/make/site.json`.
 *
 * That file was imported statically, so anywhere else — a laptop, GitHub
 * Actions, any CI — `vite build` died at config load with
 * "Cannot find module './.figma/make/site.json'" before touching a single
 * component, making the app undeployable outside Figma.
 *
 * Loading it through the filesystem with a null fallback keeps the Figma Make
 * integration working exactly as before when the file is present, and reduces
 * the config to plain React + Tailwind when it is not.
 */
/* Resolve from this config file's own directory, not the shell's cwd, so the
   alias and the Figma lookup behave the same however Vite is invoked. */
const projectRoot: string = import.meta.dirname ?? process.cwd()

function loadSiteConfiguration(): FigmaSiteConfiguration | null {
  const file = path.resolve(projectRoot, ".figma/make/site.json")
  try {
    if (!fs.existsSync(file)) return null
    return JSON.parse(fs.readFileSync(file, "utf8")) as FigmaSiteConfiguration
  } catch {
    return null // a malformed file must never make the project unbuildable
  }
}
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      tailwindcss(),
      stripFigmaMarkers(),
      // Figma Make's own plugins, applied only where Figma Make is driving.
      ...(siteConfiguration
        ? [
...
/**
 * Figma Make substitutes its own values at `<!-- figma:... -->` markers in
 * index.html. Anywhere those markers survive they are shipped verbatim — which
 * leaked the literal comment into the page title and left `lang=""` on the
 * html element. This runs after Figma's own pass (order: 'post'), so it only
 * ever removes markers Figma did not claim.
 */
function stripFigmaMarkers(): Plugin {
  return {
    name: "strip-figma-markers",
    transformIndexHtml: {
      order: "post",
      handler(html) {
        return html
          .replace(/lang="\s*<!--\s*figma:lang\s*-->\s*"/g, 'lang="en"')
          .replace(/<!--\s*figma:[a-z-]+\s*-->/g, "")
      },
    },
  }
}
