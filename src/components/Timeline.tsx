...
export function Education() {
  ...
  return (
    <Section id="education" index="02" eyebrow="Academic Record" title="Education">
...
export function Experience() {
  const { data } = usePortfolio();
  return (
    <Section id="experience" index="03" eyebrow="Field & Research" title="Experience">
...
function TimelineList({
  ...
  items: { id: string; title: string; org: string; period: string; detail: string }[];
  accent: 'neon' | 'gold';
}) {
    ...
    <ol className="relative ml-3 border-l border-hair">
      {items.map((it) => (
        <li key={it.id} className="reveal relative pb-12 pl-8 last:pb-0">
          ...
          <p className={`mt-1 text-sm font-medium ${accent === 'neon' ? 'text-sage-light' : 'text-gold-light'}`}>
            {it.org}
          </p>
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-mist">{it.detail}</p>
        </li>
      ))}
    </ol>
  );
