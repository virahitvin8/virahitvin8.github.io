        ...
        {data.projects.map((p) => (
          ...
          >
            <div
              ...
              style={{
                background:
                  "radial-gradient(400px circle at 50% 0%, rgba(0,255,200,0.08), transparent)",
              }}
            />
            <div className="relative flex items-start justify-between">
              ...
              {p.link ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-faint transition group-hover:text-neon"
                >
            ...
            <h3 className="relative mt-4 font-display text-2xl text-ink">
              {p.title}
            </h3>
            <p className="relative mt-3 flex-1 text-sm leading-relaxed text-mist">
              {p.description}
            </p>
            <div className="relative mt-5 flex flex-wrap gap-2">
              {p.tags.map((t) => (
                <span
                  key={t}
                  className="rounded-full border border-hair px-3 py-1 font-mono text-[11px] text-sage-light"
                >
                  {t}
                </span>
              ))}
      ...
      <div className="reveal mt-16">
        ...
        {error && (
          <p className="text-sm text-mist">
            Couldn&apos;t reach the GitHub API right now ({error}). Repositories
            refresh on reload.
          </p>
        )}
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {loading &&
            [0, 1, 2].map((i) => (
              <div
                key={i}
                className="h-44 animate-pulse rounded-2xl bg-white/[0.03]"
              />
            ))}
