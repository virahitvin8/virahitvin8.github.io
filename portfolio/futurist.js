...
;(function () {
  ...
  /* Boot terminal is skipped on repeat visits inside the same session. */ // slug -> data URL (admin uploads)

  /* ═══════════════════════════════════════════════════════════════
     2. HELPERS
  ═══════════════════════════════════════════════════════════════ */

  /* Fold a certificate title to something comparable.

     Titles reach this from two places that word them differently: the card's own
     `.cert-title`, and the legacy `showCertModal(title, …)` strings still baked
     into the orbit nodes and card buttons — which write "and" where the card
     writes "&". Comparing the two forms literally meant the lookup missed the
     card, and the viewer fell back to a slugified title that matches no file on
     disk, so a certificate that HAS a scan rendered the generated fallback.
     Folding punctuation and "&" → "and" makes the two forms agree.

     Only ever used on certificate titles. */
  ...
  /* WMO weather codes → our six conditions */ // windy + clear handled by data-wind
  // last condition observed without a manual override
  ...
  /* live weather — geolocation permission is remembered, never nagged */ // owner took manual control mid-flight

  /* reverse geocode for a friendly place label (free, key-less) */ /* label is a nicety, never a requirement */ /* stay silent: the clock palette is already applied */
  ...
  /* don't replay the intro on every internal navigation — and never fight
       the inline safety net that may have already released the overlay */ // hard ceiling — never trap a visitor
  ...
  ═══════════════════════════════════════════════════════════════ */ // guards against overlapping opens
  ...
  /* A card whose certificate is issued on request has no document to
           lock, so the whole-card tap target is skipped: it would otherwise
           open the viewer and the viewer would have to invent a credential. */
  ...
  /* Neutralise programmatic canvas scraping — scoped to this viewer so
       nothing else on the page (charts, sprites) is affected. */ // a newer open() superseded this one
  ...
  /* split into odd / even scanline layers + one complete layer */ // keeps odd rows
  // keeps even rows

  /* ── deterrence shield ── */ /* mac print */
  ...
  /* Track the admin bar both ways. Enabling only on activation used to
         ...
         on the owner's machine would see an editable-looking page. */ // text leaves only
  ...
  /* No scan is coming for these, so offering an upload box and probing the
         folder would report a permanent, misleading "fallback". */
  ...
  const CONFIG = {
    ...
    LINKEDIN_POSTS: [
      {
        text: "Trained at CSIR-NGRI, Hyderabad — Applications of Remote Sensing & GIS in Earth Surface Processes.",
        date: "JUL 2026",
      },
      {
        text: "Completed the two-week FDP on Next-Generation Remote Sensing Data Analytics & Multi-Domain Applications.",
        date: "AUG 2026",
      },
      {
        text: "M.Sc Remote Sensing & GIS at SHUATS — 1st semester CGPA 10.0 / 10.",
        date: "2025 – 2027",
      },
      {
        text: '"Unlocking the HCN Content in Sorghum" — published in Agri Express.',
        date: "APR 2024",
      },
    ],
    CERT_IMAGES: {},
  ...
  function renderWeatherChip() {
    ...
    chip.title =
      ...
      "auto"
        ...
        : 'Previewing "' +
          w.label +
          '" — click to cycle, or advance to AUTO to return to live conditions'
  }
  ...
  function initAdaptiveTheme() {
    ...
    if (
      ...
      Theme.setWeather(condOverride, { source: "manual" })
    if (
      themeOverride ===
        "night" ||
      themeOverride ===
  ...
  function fetchLocalWeather(silent) {
    ...
    function markNoGeo() {
      ...
      setBootLine(
        '> local observation denied — clock palette engaged <span class="ok">OK</span>',
      )
    }
  }
  ...
  function guessLocationDenied() {
    return (
      "Location off — the palette follows your local clock. " +
  ...
  async function loadWeatherFor(lat, lon, silent) {
    setBootLine(
      '> acquiring local observation ....... <span class="ok">…</span>',
    )
    try {
      const url =
      ...
      setBootLine(
        '> local observation offline — clock palette active <span class="ok">OK</span>',
      )
      if (!silent) {
      }
    }
  ...
  const WeatherFX = {
    ...
    colors() {
      ...
      return {
        a1: (
          cs.getPropertyValue("--fx-a1") ||
          "#00ffc8"
        ).trim(),
        a2: (
          cs.getPropertyValue("--fx-a2") ||
          "#c9a84c"
        ).trim(),
        a3: (
          cs.getPropertyValue("--fx-a3") ||
          "#52b788"
        ).trim(),
        night:
  ...
  function initMagnetic() {
    ...
    $$(".btn,.nav-cta,.social-btn").forEach((b) => {
      b.addEventListener("mousemove", (e) => {
        ...
        b.style.transform =
          "translate(" +
          ((e.clientX - r.left - r.width / 2) * 0.22).toFixed(1) +
          "px," +
    ...
    switch (e.type) {
      ...
      case "CreateEvent":
        return (
          ...
          esca(a) +
          "</b> created " +
          esca((e.payload && e.payload.ref_type) || "repository") +
          " <b>" +
          esca((e.payload && e.payload.ref) || short) +
          "</b>"
        )
      case "PullRequestEvent":
        return (
          ...
          esca(a) +
          "</b> " +
          esca((e.payload && e.payload.action) || "opened") +
          " a pull request in <b>" +
          esca(short) +
          "</b>"
        )
  ...
  function ghCacheRead(user) {
    try {
      const c = JSON.parse(
        sessionStorage.getItem("fx_gh_cache") ||
          "null",
      )
      if (
        !c ||
  ...
  function ghCacheWrite(user, data) {
    try {
      sessionStorage.setItem(
        "fx_gh_cache",
        JSON.stringify({ t: Date.now(), user: user, data: data }),
      )
    } catch (e) {}
  }

  function ghUser() {
    return (store.get(LS.ghUser) || CONFIG.GH_USER || "").trim()
  }
  function liUrl() {
    return (store.get(LS.liUrl) || CONFIG.LINKEDIN_URL || "").trim()
  }
  function liPosts() {
    ...
    try {
      ...
      if (!data) {
        ...
        if (
          ...
          box.innerHTML =
            '<div class="feed-empty">// no such GitHub user: <b>' +
            esca(user) +
            "</b><br>check the username in Admin → 2080 Core</div>"
          return
        }
        if (
          userRes.status ===
          ...
          box.innerHTML =
            '<div class="feed-empty">// GitHub rate limit reached for this network<br>' +
          ...
          repos
            .map(
              (r) =>
                ...
                (r.description
                  ? " — " +
                    esca(String(r.description).slice(0, 88))
                  : "") +
                "</p>" +
                "<small>" +
                (r.language
                  ? langDot(r.language) + esca(r.language) + " • "
                  : "") +
            ...
            .map(
              (e) =>
                ...
                (GH_ICONS[e.type] ||
                  "fa-bolt") +
                '"></i></div>' +
                '<div class="feed-body"><p>' +
                ghDescribe(e, user) +
                "</p>" +
                "<small>" +
                esca((e.repo && e.repo.name) || "") +
                " • " +
      ...
      box.innerHTML =
        '<div class="feed-empty">// uplink blocked (offline or rate-limited)<br>' +
  ...
  const Secure = {
    ...
    init() {
      ...
      $$(".cert-card").forEach((card) => {
        if (card.hasAttribute("data-cert-request")) return
      ...
      window.closeSecureCert = function () {
        Secure.close()
      }
    },
    guardCanvasExports() {
    ...
    async render(data) {
      ...
      if (img) {
        ...
        off.width = Math.round(
          img.naturalWidth *
            scale,
        )
        off.height = Math.round(
          img.naturalHeight *
            scale,
        )
  ...
  function certDataFromCard(card) {
    ...
    const dateEl =
      card.querySelector(".cert-date span") ||
      card.querySelector(".cert-date")
    ...
    return {
      ...
      date: date,
      slug:
        card.getAttribute("data-cert-slug") ||
        slugify(title),
      img:
        card.getAttribute("data-cert-img") ||
        "",
    }
  }
  ...
  function findCertCardByTitle(title) {
    ...
    $$(".cert-card").forEach((card) => {
      ...
      if (
        ...
        !best &&
        (t.indexOf(want.slice(0, 34)) ===
          0 ||
          want.indexOf(t.slice(0, 34)) ===
            0)
      )
        best = card
    })
  ...
  function drawGeneratedCert(cv, data, sessionId) {
    ...
    c.fillText(
      "◈  S E C U R E D   C R E D E N T I A L   V I E W E R  ◈",
      W /
        2,
      108,
    )
    ...
    c.lineWidth = 3
    c.beginPath()
    c.arc(sx, sy, 64, 0, 6.2832)
    c.stroke()
    c.setLineDash([4, 5])
    c.beginPath()
    c.arc(sx, sy, 52, 0, 6.2832)
    c.stroke()
    c.setLineDash([])
    c.fillStyle = "#c9a84c"
    c.font = "800 30px Outfit, sans-serif"
    c.fillText(
      CONFIG.OWNER_INITIALS,
      sx,
      sy +
        6,
    )
    ...
    c.font = '11px "JetBrains Mono", monospace'
    c.fillText(
      "C R E D E N T I A L   H O L D E R",
      270,
      H -
        168,
    )
    ...
    c.fillText(
      "viewer v2.080 • rendered client-side • " +
        (sessionId ||
          ""),
      W /
        2,
  ...
  const InlineEdit = {
    ...
    TEXT_TAGS:
      "h1,h2,h3,h4,h5,h6,p,span,a,li,strong,em,small,blockquote,button,label,div",
    ...
    enable() {
      ...
      $$(this.TEXT_TAGS).forEach((el) => {
        ...
        if (!el.hasAttribute("data-iedit"))
          el.setAttribute("data-iedit", elPath(el))
        el.addEventListener("dblclick", this.startTextEdit)
      })

      $$("img").forEach((img) => {
        if (img.closest(this.SKIP)) return
        if (!img.hasAttribute("data-iimg"))
          img.setAttribute("data-iimg", elPath(img))
    ...
    hint(show) {
      ...
      if (show) {
        if (!h) {
          ...
          h.textContent =
            "ADMIN LIVE EDIT • double-click text or an image • Ctrl+Shift+E to exit"
    ...
    key(el) {
      return (
        el.getAttribute("data-iedit") ||
        el.getAttribute("data-iimg") ||
        elPath(el)
      )
    },
    ...
    startTextEdit(e) {
      ...
      const finish = () => {
        ...
        toast(
          ok
            ? "Saved ✓ — included in your next HTML export"
    ...
    pickImage(e) {
      ...
      input.onchange = () => {
        ...
        compressImage(file, 1600, 0.86).then((dataUrl) => {
          ...
          toast(
            ok
              ? "Image replaced ✓ — baked into your next export"
    ...
    applySaved() {
      ...
      Object.keys(texts).forEach((key) => {
        const el =
          document.querySelector('[data-iedit="' + cssEscape(key) + '"]') ||
          findByKey(key)
      ...
      Object.keys(imgs).forEach((key) => {
        const el =
          document.querySelector('[data-iimg="' + cssEscape(key) + '"]') ||
          findByKey(key)
  ...
  function findByKey(key) {
    ...
    const all = $$(
      ".hero-name,.logo-text,.hero-tagline,.section-title,.cert-title,.exp-role,.project-title,.timeline-title,.hero-photo,.nav-avatar",
    )
  ...
  function renderCertRows() {
    ...
    if (!cards.length) {
      host.innerHTML =
        '<p class="admin-help-text">No certificate cards found.</p>'
      return
    }
    ...
    host.innerHTML = cards
      .map((card, i) => {
        ...
        if (card.hasAttribute("data-cert-request")) {
          return (
            "" +
            '<div class="admin-cert-item" style="flex-direction:column;align-items:stretch;gap:.35rem;">' +
            '<div class="admin-cert-info"><h4>' +
            esca(data.title.slice(0, 62)) +
            "</h4>" +
            "<p>" +
            esca(data.issuer.slice(0, 48)) +
            "</p></div>" +
            '<span style="font-size:.68rem;opacity:.8;color:#c9a84c;">' +
            '<i class="fas fa-envelope"></i> on request — no scan expected</span>' +
            "</div>"
          )
        }
        ...
        return (
          ...
          esca(data.issuer.slice(0, 48)) +
          "</p></div>" +
          '<input type="text" data-cert-path="' +
          esca(slug) +
          '" ' +
          'value="' +
          esca(path) +
          '" placeholder="assets/certs/' +
          esca(slug) +
          '.png" ' +
          ...
          'data-cert-upload="' +
          esca(slug) +
          '"><i class="fas fa-upload"></i> Upload scan</button>' +
          (hasUpload
    ...
    host.querySelectorAll("[data-cert-upload]").forEach((btn) => {
      btn.addEventListener("click", () =>
        uploadCertFile(btn.getAttribute("data-cert-upload")),
      )
    })
  ...
  function updateStorageNote() {
    ...
    try {
      ...
      note.textContent = bytes
        ? "Uploaded scans are using ~" +
          mb +
          " MB of this browser's ~5 MB limit. " +
    ...
    input.onchange = () => {
      ...
      compressImage(file, 1700, 0.84).then((dataUrl) => {
        ...
        if (!store.setJson(LS.certUpload, ups)) {
          if (prev) ups[slug] = prev
          else delete ups[slug]
          store.setJson(LS.certUpload, ups)
          toast(
            "Browser storage is full — drop the file into assets/certs/ instead.",
          )
        } else {
  ...
  function wirePanel(content) {
    ...
    if (gh)
      gh.addEventListener("change", () => {
        ...
        if (v) store.set(LS.ghUser, v)
        else store.del(LS.ghUser)
        loadGitHub()
      })
    ...
    content.addEventListener("change", (e) => {
      ...
      if (pathInput) {
        ...
        toast(
          v
            ? "Path saved for that certificate."
            : "Path cleared — using auto-detect.",
        )
        renderCertRows()
      }
    })
  ...
  function openAdminPanel(which) {
    ...
    const tab = $(
      '.admin-tab[data-tab="tab' +
        (which === "core" ? "Core" : "Profile") +
        '"]',
    )
    if (tab) tab.click()
  }
  window.fxCore = {
    save() {
      ...
      if (li) {
        ...
        if (v) store.set(LS.liUrl, v)
        else store.del(LS.liUrl)
      }

      const lp = $("#coreLiPosts")
      if (lp) {
        const v = lp.value.trim()
        if (v) store.set(LS.liPosts, v)
        else store.del(LS.liPosts)
      }
      ...
      if (th) {
        if (th.value === "auto") {
          store.del(LS.theme)
          Theme.state.theme = Theme.state.band === "night" ? "night" : "day"
        } else {
          store.set(LS.theme, th.value)
          Theme.state.theme = th.value
        }
      }
      ...
      if (w) {
        if (w.value === "auto") {
          store.del(LS.weather)
          Theme.state.mode = "auto"
          Theme.setWeather(Theme.state.autoWeather || "clear", {
            source: "clock",
          })
          Theme.setBand(timeBand(new Date()))
          fetchLocalWeather(true)
      ...
      if (typeof window.portfolioAdmin !== "undefined") {
        ...
        if (t && toastEl) {
          t.textContent = "2080 settings applied ✓"
  ...
  function toast(msg) {
    ...
    if (!t) {
      ...
      t.id = "fxToast"
      t.style.cssText =
        "position:fixed;bottom:7.4rem;left:50%;transform:translateX(-50%);" +
